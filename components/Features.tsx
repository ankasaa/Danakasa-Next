      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {features.map((feature, index) => (
          <Reveal key={feature.title} delay={index * 0.08} className="h-full">
            <article className="flex h-full flex-col rounded-3xl bg-white p-8 text-left shadow-sm ring-1 ring-neutral-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-lg">
              <Image
                src={feature.icon}
                alt={feature.title}
                width={45}
