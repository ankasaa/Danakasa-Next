      ...
      <div className="mt-14 grid gap-6 sm:grid-cols-3">
        {tools.map((tool) => (
          <Link
            key={tool.href}
            href={tool.href}
            className="group flex flex-col rounded-3xl bg-white p-8 shadow-sm ring-1 ring-neutral-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-lg"
          >
            ...
            <span className="mt-5 inline-block text-sm font-medium text-brand-500">
              Segera hadir →
            </span>
          </Link>
        ))}
